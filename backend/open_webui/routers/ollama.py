...
async def get_all_models(request: Request, user: UserModel | None = None):
    ...
    # Post-process each response: apply prefix_id, tags, model filtering. Also
    # fetch /api/show once per cached model-list refresh so callers can avoid
    # sending native tools to completion-only GGUF models.
    capability_targets: list[dict] = []
    capability_tasks = []
    for idx, response in enumerate(responses):
        ...
        for m in response.get('models', []):
            raw_model_id = m['model']
            capability_targets.append(m)
            capability_tasks.append(
                send_request(
                    f'{url}/api/show',
                    payload=json.dumps({'model': raw_model_id}),
    ...
    if capability_tasks:
        capability_responses = await asyncio.gather(*capability_tasks, return_exceptions=True)
