<script lang="ts">
	...
	const KOKORO_VOICES = {
		af_heart: { name: 'Heart' },
		af_bella: { name: 'Bella' },
		af_nicole: { name: 'Nicole' },
		af_sarah: { name: 'Sarah' },
		am_fenrir: { name: 'Fenrir' },
		am_michael: { name: 'Michael' },
		bf_emma: { name: 'Emma' },
	...
	const loadKokoro = async () => {
		if (TTSEngine === 'browser-kokoro') {
			...
			if (TTSEngineConfig?.dtype) {
				// Voice metadata is bundled; the q8 inference model remains lazy-loaded
				// by KokoroWorker only when speech is requested.
				TTSModel = { voices: KOKORO_VOICES };
