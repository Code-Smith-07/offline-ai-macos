<script lang="ts">
		...
		if ($showCallOverlay) {
			return new Promise((resolve, reject) => {
				...
				if (audioElement) {
					...
					audioElement.onended = async (event) => {
						await new Promise((r) => setTimeout(r, 100));
						resolve(event);
					};
					audioElement.onerror = () => {
						reject(new Error(audioElement.error?.message ?? 'Voice audio playback failed'));
					};

					audioElement.play().catch(reject);
				} else {
					reject(new Error('Voice audio output is unavailable'));
				}
			});
		} else {
			return Promise.resolve();
	...
	const stopAllAudio = async () => {
		...
		if (audioElement) {
			audioElement.pause();
			audioElement.currentTime = 0;
			// The element is shared with Read Aloud. Never leave it muted after
			// voice mode closes or subsequent synthesized speech will be silent.
			audioElement.muted = false;
		}
	};
