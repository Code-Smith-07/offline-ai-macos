<script lang="ts">
	...
	let modelLoadObservedGeneration = false;
	let modelLoadTimeout: number | undefined;

	const clearModelLoadRequest = () => {
		modelLoadRequested = false;
		modelLoadObservedGeneration = false;
		if (modelLoadTimeout !== undefined) {
			window.clearTimeout(modelLoadTimeout);
			modelLoadTimeout = undefined;
		}
	};

	const submitCurrentPrompt = async () => {
		const selectedOllamaModel = selectedModelIds.some(
			(modelId) => $models.find((model) => model.id === modelId)?.owned_by === 'ollama'
		);

		if (selectedOllamaModel && !isActive) {
			clearModelLoadRequest();
			modelLoadRequested = true;
			modelLoadTimeout = window.setTimeout(clearModelLoadRequest, 120_000);

			// Give Svelte and the browser one frame to paint "Loading model…" before
			// dispatching the message and starting the potentially expensive request.
			await tick();
	...
	$: if (modelLoadRequested && isActive) {
		modelLoadObservedGeneration = true;
	}

	$: if (modelLoadRequested && modelLoadObservedGeneration && !isActive) {
		clearModelLoadRequest();
	}

	onDestroy(clearModelLoadRequest);
	...
	const screenCaptureHandler = async () => {
		try {
			if (!navigator.mediaDevices?.getDisplayMedia) {
				toast.error(
					$i18n.t(
						'Screen capture is unavailable. Allow Screen Recording for Offline AI in System Settings.'
					)
				);
				return;
			}
			...
			if (!context) {
				throw new Error($i18n.t('Unable to create the screen capture image.'));
			}
			...
			if (error?.name !== 'NotAllowedError' || `${error}`.toLowerCase().includes('denied')) {
				toast.error(
					$i18n.t(
						'Screen capture failed. Allow Screen Recording for Offline AI in System Settings, then try again.'
					)
				);
			}
		}
						...
						>
							...
							<div class=" flex justify-between mt-0.5 mb-2 mx-0.5 max-w-full" dir="ltr">
								...
								<div class="self-end flex space-x-1 mr-1 shrink-0 gap-[0.5px]">
									<div class="flex min-w-0 max-w-[18rem] items-center sm:max-w-[24rem]">
										<ModelSelector
											bind:selectedModels
											generationActive={isActive || modelLoadRequested}
														...
														on:click={async () => {
															try {
																if (!navigator.mediaDevices?.getUserMedia) {
																	toast.error(
																		$i18n.t('Microphone access is unavailable in this app.')
																	);
																	return;
																}
