...
export const getOllamaLoadedModels = async (token: string = '') => {
...
export const preloadOllamaModel = async (
	token: string = '',
	model: string,
	keepAlive: string | number = '10m'
) => {
	...
	// The Open WebUI Ollama proxy returns this endpoint as a streamed response
	// even when Ollama's payload requests stream=false. Consume it fully so the
	// model has finished loading before the UI changes to "Model active".
	await res.text();
	return true;
};
